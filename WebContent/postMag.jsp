...
<title>MagicBBS: Post Manager</title>
...
<jsp:include page="common/NavigationBar.jsp"></jsp:include>
	...
	<div class="submit-ad main-grid-border">
		<div class="container">
			<h2 class="head" style =" font-family: 'Comic Sans MS'">Your Posts </h2>
		</div>
	</div>
 ...
 <div class="page-header"></div>
  <div class="panel-body"></div>
		 ...
		 <div class="col-lg-6">
    		...
    		<form id="search" action = "${ pageContext.request.contextPath}/postMag_Servlet.do?&un=${un}" class = "navbar-form narbar-left" role="search" method="post">
    		  <input type = "hidden"  name = "m" id="m"  value = "search"/>
    		  <div class = "form-group">
    		  		<h5 >Title</h5>
    		  		<input type="text" name = "title"  id="title" class="form-control" placeholder="Search by Post Title..." value="${title }">&nbsp;&nbsp;&nbsp;
    		  </div>
    		  <div class = "form-group">
    		  		<h5>&nbsp;</h5>
        		<button type = "submit" class="btn btn-default" type="button">Query</button>&nbsp;&nbsp;
        		</div>
        	  <div class = "form-group">
    		  		<h5>&nbsp;</h5>
    		  		<input type = "hidden"  name = "pIds" id="pIds"  />
        		<button type = "submit"class="btn btn-danger" type="button" onclick="getCheckboxVal(); return deleteWarning();">Batch Delete</button>
        		</div>
        		&nbsp;&nbsp;&nbsp;
     		</form>
    		</div><!-- /input-group -->
  ...
  <thead>
		<tr>
				<th width = "5%">
					<input type = "checkbox" id = "allselect" onclick="checkAll();">
				</th>
				<th>ID</th>
				<th>Title</th>
				<th>Content</th>
		...
		<tbody id = "dataTable">
				<c:forEach items="${postList}" var = "posts" varStatus = "s">
								<tr>
										<td>
											<input type="checkbox" name = "chkinstrid" value = " ${posts.id }">
										</td>
										<td>${s.count}</td>
									...
									<td>
											...
											<a href="${ pageContext.request.contextPath}/	postMag_Servlet.do?id=${posts.id }&m=edit&un=${un }" role="button" data-toggle="modal">Revise</a>	&nbsp;&nbsp;
											<a href="${ pageContext.request.contextPath}/postDelete.do?id=${posts.id }&un=${un}" onclick="return deleteWarning();" role="button" data-toggle="modal" style="color:red;">Delete</a>
									</td>
								</tr>			
							</c:forEach >	
							</tbody>
